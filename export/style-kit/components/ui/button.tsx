...
const getAnimationConfig = (variant: string | null | undefined) => {
  // Link variant doesn't need scale animation
  if (variant === "link") {
    return {
      whileHover: {},
      whileTap: {},
  ...
  // Ghost and outline get subtle animation
  ...
  // Default, secondary, destructive get standard animation
  return {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, disableAnimation = false, ...props }, ref) => {
    // If asChild is true, use Slot (no animation support with Slot)
    if (asChild) {
      return (
        <Slot
          data-slot="button"
    ...
    // If animation is disabled, use regular button
    if (disableAnimation) {
      return (
        <button
          data-slot="button"
    ...
    const animationConfig = getAnimationConfig(variant)
